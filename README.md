...
|----------------|-------|-----------------------------------------|
|                |Current master branch    |Fixed bugs:  <li>race of queued inter-thread signals and/or queued GUI events, set widget attributes;<li>segmentation on a clear command for listbox widget with selection option enabled; <li>the first item added to a combobox/dropdown widget with selection option is reported as empty;
